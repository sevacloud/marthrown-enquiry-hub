...
- `selected_dates` holds one or more candidate dates per enquiry.
...
- **Enquiry**: A single event enquiry submitted by one person at one point in time, with its own lifecycle, candidate dates, notes and history. The unit of work for the management team.
- **Contact**: A person record held in FluentCRM, identified by email address. One Contact relates to zero or more Enquiries.
- **Candidate Date**: One date a person has indicated as a possible date for the event described by an Enquiry. An Enquiry holds one or more Candidate Dates.
...
- **Manual Validation Profile**: The Validator required-field profile that treats `first_name`, `last_name`, `email` and `selected_dates` as required and treats `phone`, `total_guests`, `message`, `event_type` and `site_exclusivity` as optional. Applied to a submission arriving through the manual enquiry creation route or the enquiry edit route.
...
- **Enquiry_Store**: The plugin-owned persistence layer for enquiries, candidate dates, notes and history.
...
2. THE Enquiry_Store SHALL persist each Candidate Date as a separate row related to exactly one Enquiry by that Enquiry's identifier, holding one calendar date at day precision, and SHALL accept between 1 and 10 Candidate Date rows for a single Enquiry.
...
6. WHEN an Enquiry is written to the Enquiry_Store and then read back by identifier, THE Enquiry_Store SHALL return every stored field value character-for-character equal to the written value, SHALL return `created_at`, `updated_at` and `status_changed_at` equal to the written date-time to the nearest second in the site timezone, and SHALL return the Candidate Dates, `event_type` values and `site_exclusivity` values as sets equal to the written sets irrespective of row order (round-trip property).
...
12. WHEN the Schema_Manager applies schema changes, THE Schema_Manager SHALL retain all existing Enquiry rows, Candidate Date rows, note rows and history rows, with every field value in those rows unchanged except where a change explicitly transforms that field.
...
15. THE Enquiry_Store SHALL index `email`, `status`, `created_at`, `fluentcrm_subscriber_id` and `is_test` on the Enquiry table and the date value on the Candidate Date table, to support the list queries defined in Requirement 12.
...
5. WHEN the Intake_Handler creates an Enquiry, THE Intake_Handler SHALL store one Candidate Date row for each date present in the submitted `selected_dates` value.
...
7. THE Settings SHALL provide a mapping control for each of the nine enquiry fields that records which Intake Webhook Request payload field supplies that enquiry field.
...
5. IF the submitted `selected_dates` value contains fewer than 1 or more than 10 dates, THEN THE Validator SHALL return a validation failure naming the `selected_dates` field.
6. IF a value in the submitted `selected_dates` value does not parse as a calendar date, THEN THE Validator SHALL return a validation failure naming the `selected_dates` field.
...
14. WHERE the submission arrived as an Intake Webhook Request, THE Validator SHALL apply the Webhook Validation Profile and SHALL treat `first_name`, `last_name`, `email`, `phone`, `total_guests`, `selected_dates`, `event_type`, `site_exclusivity` and `message` as required fields.
15. WHERE the submission arrived through the manual enquiry creation route defined in Requirement 18 or the enquiry edit route defined in Requirement 19, THE Validator SHALL apply the Manual Validation Profile and SHALL treat `first_name`, `last_name`, `email` and `selected_dates` as required fields.
...
2. WHEN a submission holds the same `email` value and the same set of Candidate Dates as an existing Enquiry created within the preceding 15 minutes, THE Duplicate_Detector SHALL report that submission as a duplicate.
...
9. THE Contact_Linker SHALL write no Candidate Date, `event_type`, `site_exclusivity` or `message` value to FluentCRM subscriber custom fields.
...
4. WHEN a new Enquiry is created from an existing Enquiry, THE Enquiry_Store SHALL copy `first_name`, `last_name`, `email`, `phone`, `total_guests`, `message`, the Candidate Dates, the `event_type` values and the `site_exclusivity` values from the source Enquiry.
...
7. WHEN both a `date_from` and a `date_to` parameter are supplied, THE Enquiry_API SHALL return only Enquiries holding at least one Candidate Date within that range inclusive.
8. IF only one of `date_from` and `date_to` is supplied, THEN THE Enquiry_API SHALL apply no Candidate Date filter and SHALL return a warning naming the missing parameter.
...
2. WHEN a single Enquiry is returned, THE Enquiry_API SHALL include the stored field values, every Candidate Date, every `event_type` value, every `site_exclusivity` value, the notes, the history entries, the `crm_sync_state`, the linked booking identifier and the FluentCRM contact URL.
...
1. THE Enquiry_API SHALL expose a route that creates a Booking from an Enquiry, accepting a target calendar identifier and a chosen Candidate Date.
2. WHEN a Booking is created from an Enquiry, THE Booking_Creator SHALL set the Booking start date and end date from the chosen Candidate Date.
...
4. WHEN a Booking is created from an Enquiry, THE Booking_Creator SHALL block the Booking date range on the target calendar using that calendar's booked legend item.
...
7. WHEN a Booking is created from an Enquiry, THE History_Recorder SHALL record a history entry of type `booking_linked` holding the booking identifier.
...
10. WHEN the Enquiry_API creates an Enquiry from a manual enquiry creation request, THE Enquiry_Store SHALL store one Candidate Date row for each date present in the submitted `selected_dates` value, one row for each submitted `event_type` value and one row for each submitted `site_exclusivity` value.
...
13. WHEN the Enquiry_API receives a manual enquiry creation request holding the same `email` value and the same set of Candidate Dates as an existing Enquiry created within the preceding 900 seconds, THE Enquiry_API SHALL create a new Enquiry.
...
1. THE Enquiry_API SHALL expose an enquiry edit route that updates the `first_name`, `last_name`, `email`, `phone`, `total_guests`, `message`, Candidate Dates, `event_type` values and `site_exclusivity` values of an existing Enquiry.
