 ...
 *
 * Renders bare, as a run of inline controls: `HubTable`'s toolbar row is what
 * lays them out and adds the Export CSV link, matching the search section
 * `BookingsManager` renders through the same slot.
 *
 ...
 *
 * The hide-test checkbox starts unticked, leaving `hide_test` falsy so test
 * enquiries are listed until the user asks for them to be hidden (Requirement
 * 17.6). A checkbox rather than a toggle switch, matching "Hide past bookings"
 * on the bookings list — the two are the same kind of control and read
 * oddly as two different ones.
 */
...
export default function EnquiryFilters( { filters, onChange } ) {
	...
	return (
		<>
			...
			<CheckboxControl
				label={ __( 'Hide test enquiries', 'marthrown-enquiry-hub' ) }
